...
Every line of terminal output on the page comes from `estimated` itself; the
...
point this at a checkout of it and commit the data.js it writes:
    ...
    python build.py --project ../estimated
"""
...
def locate(given: str | None) -> Path:
    """The estimated checkout: --project, or a folder named estimated next to this one."""
    for candidate in [Path(given)] if given else [HERE.parent / "estimated"]:
        if (candidate / "src" / "estimated" / "__init__.py").is_file():
            return candidate.resolve()
    raise SystemExit("estimated not found: pass --project PATH, the folder that holds src/estimated")


_options = argparse.ArgumentParser(description="Build data.js from the real program.")
_options.add_argument("--project", help="a checkout of github.com/Artemoon13/estimated")
ROOT = locate(_options.parse_args().project)
sys.path.insert(0, str(ROOT / "src"))

from estimated import __version__, config, demo, git, render  # noqa: E402
from estimated.__main__ import main  # noqa: E402
from estimated.analysis import analyze  # noqa: E402
from estimated.i18n import STRINGS, Lang, by, times  # noqa: E402
from estimated.pair import Keys, collect  # noqa: E402
from estimated.sources import ledger  # noqa: E402

SHOWN = "/tmp/estimated-demo"  # where `estimated demo` puts it on Linux; the page shows that path
EN = Lang("en")
...
def films(run: Program, free: int, example: str) -> list[dict]:
    ...
    return [
        film("say", "writing a promise down", "estimated say",
             (f"estimated say {key} 2d", said), (f"estimated say {key} 3d", again),
             (f"estimated say {other} soon", vague)),
        film("one task", "taken apart", f"estimated {example}", (f"estimated {example}", body)),
        film("doctor", "through the monocle", "estimated doctor", ("estimated doctor", doctor)),
    ]


def toy_ledger() -> dict:
    """The four-line ledger on the page, written and checked by the real ledger code."""
    root = Path(tempfile.mkdtemp(prefix="estimated-toy-"))
    try:
...
def build() -> dict:
    work = Path(tempfile.mkdtemp(prefix="estimated-site-"))
    try:
        path = work / "estimated-demo"
        demo.build(path)
        run = Program(path)
...
if __name__ == "__main__":
    ...
    target.write_text("// Generated by site/build.py from the real program. Do not edit by hand.\n"
                      f"window.ESTIMATED = {json.dumps(data, ensure_ascii=False, separators=(',', ':'))};\n",
                      encoding="utf-8")
